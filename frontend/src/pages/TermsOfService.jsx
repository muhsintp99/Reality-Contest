...
import { FileText, ArrowLeft, Home, Shield, Scale, AlertTriangle, CheckCircle, Mail, Printer } from 'lucide-react';
...
export const TermsOfService = () => {
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      try {
        const res = await axios.get('/api/cms/terms', { timeout: 4000 });
        if (res.data?.document) {
          const loadedDoc = res.data.document;
          setDoc({
            title: loadedDoc.title || 'Terms of Service',
        ...
        console.log('Terms of Service CMS load info:', err?.message);
      } finally {
        setLoading(false);
      }
    };
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12 space-y-8">
        ...
        <div className="bg-gradient-to-r from-[#1C1335] via-[#2E1E54] to-[#1C1335] border border-[#CEF500]/30 rounded-3xl p-8 sm:p-10 shadow-2xl relative overflow-hidden text-left space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[11px] font-black text-[#CEF500] uppercase tracking-wider">Legal Terms & Conditions</span>
              <h1 className="text-3xl sm:text-4xl font-extrabold text-white font-poppins">{doc.title || 'Terms of Service'}</h1>
            </div>
          </div>
          <p className="text-xs sm:text-sm text-[#A69EC6] max-w-2xl leading-relaxed">
            Please read these Terms of Service carefully before creating an account or participating in contests, room cycles, and reward programs on Reality Contest Platform.
          </p>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-6 text-xs text-white/70 font-semibold border-t border-white/10">
            ...
            <span>Governance: <strong className="text-emerald-400">Fair Play Guidelines</strong></span>
          </div>
        </div>
        ...
        <div className="bg-[#1C1335]/70 border border-[#2E1E54] rounded-3xl p-8 sm:p-10 text-left space-y-8 shadow-xl">
          {loading ? (
            <div className="py-16 text-center space-y-4">
              ...
              <p className="text-xs font-bold text-white/70">Loading Terms of Service...</p>
            </div>
          ) : doc.content ? (
            <div
            ...
            <div className="py-12 px-6 text-center space-y-2 bg-[#0D0714]/60 border border-[#2E1E54] rounded-2xl">
              <p className="text-sm font-bold text-white">No official Terms of Service document currently published.</p>
