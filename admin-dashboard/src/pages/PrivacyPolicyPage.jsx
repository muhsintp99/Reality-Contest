...
import { Shield, Edit, Eye, Save, RefreshCw, Check } from 'lucide-react';
...
export const PrivacyPolicyPage = () => {
  ...
  const fetchDocument = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/admin/cms/doc/privacy', { withCredentials: true });
      if (res.data.success && res.data.document) {
        setDocument(res.data.document);
      }
    } catch (err) {
      console.warn('[PrivacyPolicyPage] Error fetching document:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put('/api/admin/cms/doc/privacy', formData, { withCredentials: true });
      if (res.data.success) {
        setDocument(res.data.document);
      ...
      showSnackbar('Privacy Policy updated.', 'success');
    }
    setDrawerOpen(false);
  };
  ...
  const handleSeed = async () => {
    try {
      ...
      if (res.data.success) {
        showSnackbar('Privacy Policy seeded with default content!', 'success');
        fetchDocument();
      }
    } catch (err) {
      showSnackbar('Error seeding Privacy Policy: ' + (err.response?.data?.message || err.message), 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-white/50">
            Manage data safety standards, identity verification terms, and privacy compliance rules.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            title="Seed Default Privacy Policy"
          >
            <span>⚡ Seed Privacy Policy Default</span>
          </button>
          <button
      ...
      {loading ? (
        <div className="flex items-center justify-center py-6 gap-2 text-xs text-brandPrimary font-bold animate-pulse">
          ...
          <span>Loading Privacy Policy from Backend...</span>
        </div>
      ) : (
        <div className="glassmorphism p-6 rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/80 dark:bg-slate-900/40 shadow-xl space-y-6">
          ...
          <div className="bg-slate-50/80 dark:bg-[#080b12] border border-slate-200/80 dark:border-white/10 rounded-2xl p-6 shadow-inner">
            {renderFormattedContent(document.content || 'No Privacy Policy content added yet. Click "Edit Privacy Policy" to write terms.')}
          </div>
        </div>
      )}

      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} PRIVACY POLICY`}>
        {drawerMode === 'view' ? (
          ...
          <form onSubmit={handleSave} className="space-y-4 text-left">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Policy Content (HTML / Markdown)</label>
              ...
              <div className="flex flex-wrap gap-1.5 p-2 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-white/10 text-[11px] font-mono">
                ...
                <button type="button" onClick={() => setFormData(prev => ({ ...prev, content: '<h2>1. Data Collection</h2>\n<p>We collect essential user information for KYC compliance.</p>\n\n<h2>2. Security & Storage</h2>\n<p>Bank-grade encryption applies to all stored personal data.</p>' }))} className="ml-auto px-2 py-1 bg-brandPrimary/10 text-brandPrimary font-bold border border-brandPrimary/30 rounded-lg hover:bg-brandPrimary hover:text-white transition-all cursor-pointer">⚡ Load Template</button>
              </div>
              ...
              <textarea
                ...
                placeholder="<h2>1. Overview</h2>&#10;<p>Write or paste privacy policy content here...</p>"
