      ...
      {loading ? (
        ...
        <div className="glassmorphism p-6 rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/80 dark:bg-slate-900/40 shadow-xl space-y-6">
          ...
          <div className="bg-slate-50/80 dark:bg-[#080b12] border border-slate-200/80 dark:border-white/10 rounded-2xl p-6 font-mono text-xs text-slate-800 dark:text-white/90 whitespace-pre-wrap leading-relaxed">
            {document.content || 'No Privacy Policy content added yet. Click "Edit Privacy Policy" to write terms.'}
          </div>
        </div>
      )}
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} PRIVACY POLICY`}>
        {drawerMode === 'view' ? (
          ...
          <form onSubmit={handleSave} className="space-y-4 text-left">
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Policy Content (Markdown/Text)</label>
              <textarea
                rows={12}
                required
