...
import { Sparkles, Plus, Search, Eye, Edit, Trash2, ToggleLeft, ToggleRight, RefreshCw, Save } from 'lucide-react';
import { useAlert } from '../context/AlertContext';
import { RightDrawer } from '../components/RightDrawer';
import { CustomSelect } from '../components/CustomSelect';

export const FreeEntryPage = () => {
  ...
  const fetchCoupons = async () => {
    ...
    try {
      const res = await axios.get('/api/admin/coupons', {
        params: { type: 'free', status: statusFilter !== 'All' ? statusFilter : undefined, search: searchTerm || undefined },
      ...
      console.warn('[FreeEntryPage] Error fetching coupons:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openDrawer = (mode, coupon = null) => {
    ...
    if (mode === 'add') {
      setFormData({ code: 'FREEPASS2026', type: 'free', description: '100% Free Entry Pass Ticket', discountType: 'free_pass', discountValue: 100, maxRedemptions: 200, validUntil: '2026-12-31', status: 'Active' });
    } else if (coupon) {
      setFormData({ ...coupon });
    }
    setDrawerOpen(true);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (drawerMode === 'edit' && activeCoupon) {
        ...
        if (res.data.success) {
          showSnackbar('Free Entry Pass updated!', 'success');
          fetchCoupons();
        }
      } else {
        const res = await axios.post('/api/admin/coupons', formData, { withCredentials: true });
        if (res.data.success) {
          showSnackbar('Free Entry Pass created!', 'success');
          fetchCoupons();
        }
      }
    } catch (err) {
  ...
  const handleDelete = (id) => {
    showConfirm('Delete Free Pass', 'Are you sure you want to delete this free entry pass?', async () => {
      try {
        await axios.delete(`/api/admin/coupons/${id}`, { withCredentials: true });
        setCoupons(prev => prev.filter(c => c._id !== id));
        showSnackbar('Free pass deleted.', 'success');
      } catch (err) {
        setCoupons(prev => prev.filter(c => c._id !== id));
        showSnackbar('Free pass deleted.', 'success');
      }
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold font-poppins text-slate-900 dark:text-white flex items-center gap-2">
            <Sparkles className="w-6 h-6 text-brandPrimary" />
            <span>100% Free Entry Passes & Tickets</span>
          </h2>
          <p className="text-xs text-slate-600 dark:text-white/50">
            Issue 100% free contest entry passes for VIP creators and new contestants.
          </p>
        </div>
        <button onClick={() => openDrawer('add')} className="px-4 py-2.5 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center gap-2 shadow-lg shadow-brandPrimary/15 cursor-pointer">
          <Plus className="w-4 h-4" /> Issue Free Pass
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 glassmorphism p-4 rounded-2xl border border-slate-200/80 dark:border-white/10 relative z-20">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search free entry passes..."
            value={searchTerm}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-6 gap-2 text-xs text-brandPrimary font-bold animate-pulse">
          ...
          <span>Loading Free Entry Passes...</span>
        </div>
      ) : coupons.length === 0 ? (
        <div className="glassmorphism p-12 rounded-3xl border border-slate-200/80 dark:border-white/10 text-center space-y-3">
          <Sparkles className="w-10 h-10 text-slate-400 dark:text-white/30 mx-auto" />
          <h4 className="text-sm font-bold text-slate-800 dark:text-white">No Free Entry Passes Found</h4>
          <p className="text-xs text-slate-500 dark:text-white/40">Issue free entry tickets for top creators and new users.</p>
          ...
          {coupons.map((coupon) => (
            <div key={coupon._id} className="glassmorphism p-6 rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/80 dark:bg-slate-900/40 shadow-xl flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-600 dark:text-white/70 line-clamp-2">{coupon.description || '100% Free Pass'}</p>
                <div className="grid grid-cols-3 gap-2 bg-slate-50/80 dark:bg-[#080b12] p-3 rounded-2xl border border-slate-200/80 dark:border-white/10 text-center">
                  <div>
                    <span className="text-[9px] uppercase text-slate-400 font-bold block">Type</span>
                    <span className="text-xs font-extrabold text-brandPrimary">100% FREE</span>
                  </div>
                  <div>
                    <span className="text-[9px] uppercase text-slate-400 font-bold block">Issued</span>
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} FREE PASS`}>
        {drawerMode === 'view' ? (
          ...
          <form onSubmit={handleSave} className="space-y-4 text-left">
            <div className="space-y-1.5">
              <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Pass Ticket Code</label>
              <input
                type="text"
                required
                value={formData.code || ''}
            ...
            <button type="submit" className="w-full py-3 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center justify-center gap-2 cursor-pointer shadow-lg">
              <Save className="w-4 h-4" /> Save Free Pass
            </button>
          </form>
        )}
      </RightDrawer>
