...
import { Users, Plus, Search, Eye, Edit, Trash2, ToggleLeft, ToggleRight, RefreshCw, Save } from 'lucide-react';
...
export const PartnerCampaignsPage = () => {
  ...
  const openDrawer = (mode, ad = null) => {
    ...
    setActiveAd(ad);
    if (mode === 'add') {
      setFormData({ title: '', type: 'partner', sponsorName: '', redirectUrl: '', partnerCode: '', budget: 100000, placement: 'Partner Referral Link', status: 'Active' });
    } else if (ad) {
      setFormData({ ...ad });
    }
    setDrawerOpen(true);
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold font-poppins text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span>Partner Campaigns Management</span>
          </h2>
          <p className="text-xs text-slate-600 dark:text-white/50">
            Brand partnership deals, affiliate tracking links, and conversion analytics.
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => openDrawer('add')} className="px-4 py-2.5 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center gap-2 shadow-lg shadow-brandPrimary/15 cursor-pointer">
            <Plus className="w-4 h-4" /> Add Partner Deal
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 glassmorphism p-4 rounded-2xl border border-slate-200/80 dark:border-white/10 relative z-20">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search partner campaigns..."
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="glassmorphism p-12 rounded-3xl border border-slate-200/80 dark:border-white/10 text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 dark:text-white/40">Launch brand affiliate and partnership campaigns with referral codes.</p>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-start">
                  <span className="px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase bg-brandPrimary/10 text-brandPrimary border border-brandPrimary/20">
                    Partner • {ad.partnerCode || 'AFFILIATE'}
                  </span>
                ...
                <p className="text-xs text-slate-500 dark:text-white/40 font-semibold">Partner: {ad.sponsorName || 'N/A'}</p>
                <div className="grid grid-cols-3 gap-2 bg-slate-50/80 dark:bg-[#080b12] p-3 rounded-2xl border border-slate-200/80 dark:border-white/10 text-center">
                  ...
                  <div>
                    <span className="text-[9px] uppercase text-slate-400 font-bold block">Conversions</span>
                    <span className="text-xs font-extrabold text-emerald-500">{ad.conversions || 0}</span>
                  </div>
                </div>
              </div>
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} PARTNER CAMPAIGN`}>
        {drawerMode === 'view' ? (
          <div className="space-y-4 text-left">
            ...
            <p className="text-xs text-slate-500">Partner Code: {formData.partnerCode}</p>
          </div>
        ) : (
