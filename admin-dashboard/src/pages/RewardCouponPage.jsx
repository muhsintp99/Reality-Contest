...
export const RewardCouponPage = () => {
  ...
  const fetchCoupons = async () => {
    ...
    try {
      const res = await axios.get('/api/admin/coupons', {
        params: { type: 'reward', status: statusFilter !== 'All' ? statusFilter : undefined, search: searchTerm || undefined },
      ...
      console.warn('[RewardCouponPage] Error fetching coupons:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openDrawer = (mode, coupon = null) => {
    ...
    if (mode === 'add') {
      setFormData({ code: 'REWARD100', type: 'reward', description: 'Reward Hub Redeemable Coupon', discountType: 'flat', discountValue: 100, maxRedemptions: 1000, validUntil: '2026-12-31', status: 'Active' });
    } else if (coupon) {
      setFormData({ ...coupon });
    }
    setDrawerOpen(true);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (drawerMode === 'edit' && activeCoupon) {
        ...
        if (res.data.success) {
          showSnackbar('Reward coupon updated!', 'success');
          fetchCoupons();
        }
      } else {
        const res = await axios.post('/api/admin/coupons', formData, { withCredentials: true });
        if (res.data.success) {
          showSnackbar('Reward coupon created!', 'success');
          fetchCoupons();
        }
      }
    } catch (err) {
  ...
  const handleDelete = (id) => {
    showConfirm('Delete Reward Coupon', 'Are you sure you want to delete this reward coupon?', async () => {
      try {
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold font-poppins text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span>Reward Store Coupons</span>
          </h2>
          <p className="text-xs text-slate-600 dark:text-white/50">
            Coupons redeemable in the Reward Hub using earned platform wallet points & coins.
          </p>
        </div>
        <button onClick={() => openDrawer('add')} className="px-4 py-2.5 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center gap-2 shadow-lg shadow-brandPrimary/15 cursor-pointer">
          <Plus className="w-4 h-4" /> Create Reward Coupon
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 glassmorphism p-4 rounded-2xl border border-slate-200/80 dark:border-white/10 relative z-20">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search reward coupons..."
            value={searchTerm}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-6 gap-2 text-xs text-brandPrimary font-bold animate-pulse">
          ...
          <span>Loading Reward Coupons...</span>
        </div>
      ) : coupons.length === 0 ? (
        <div className="glassmorphism p-12 rounded-3xl border border-slate-200/80 dark:border-white/10 text-center space-y-3">
          ...
          <h4 className="text-sm font-bold text-slate-800 dark:text-white">No Reward Coupons Found</h4>
          <p className="text-xs text-slate-500 dark:text-white/40">Create reward coupons redeemable using earned loyalty points.</p>
          ...
          {coupons.map((coupon) => (
            <div key={coupon._id} className="glassmorphism p-6 rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/80 dark:bg-slate-900/40 shadow-xl flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-600 dark:text-white/70 line-clamp-2">{coupon.description || 'Reward Coupon'}</p>
                <div className="grid grid-cols-3 gap-2 bg-slate-50/80 dark:bg-[#080b12] p-3 rounded-2xl border border-slate-200/80 dark:border-white/10 text-center">
                  <div>
                    <span className="text-[9px] uppercase text-slate-400 font-bold block">Value</span>
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} REWARD COUPON`}>
        {drawerMode === 'view' ? (
          ...
          <form onSubmit={handleSave} className="space-y-4 text-left">
            <div className="space-y-1.5">
              <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Reward Coupon Code</label>
              <input
                type="text"
                required
                value={formData.code || ''}
            ...
            <button type="submit" className="w-full py-3 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center justify-center gap-2 cursor-pointer shadow-lg">
              <Save className="w-4 h-4" /> Save Reward Coupon
            </button>
          </form>
        )}
      </RightDrawer>
