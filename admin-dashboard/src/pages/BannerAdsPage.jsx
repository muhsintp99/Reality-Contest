...
import { Image as ImageIcon, Plus, Search, Eye, Edit, Trash2, ToggleLeft, ToggleRight, RefreshCw, Save, Video, ExternalLink } from 'lucide-react';
import { useAlert } from '../context/AlertContext';
import { RightDrawer } from '../components/RightDrawer';
import { CustomSelect } from '../components/CustomSelect';
import { FileUploadPicker } from '../components/FileUploadPicker';

export const BannerAdsPage = () => {
  ...
  const fetchAds = async () => {
    ...
    try {
      const res = await axios.get('/api/admin/ads', {
        params: { type: 'banner', status: statusFilter !== 'All' ? statusFilter : undefined, search: searchTerm || undefined },
      ...
      console.warn('[BannerAdsPage] Error fetching ads:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (drawerMode === 'edit' && targetId) {
        ...
        if (res.data.success) {
          showSnackbar('Banner Ad updated!', 'success');
          fetchAds();
        }
      } else {
        const res = await axios.post('/api/admin/ads', payload, { withCredentials: true });
        if (res.data.success) {
          showSnackbar('Banner Ad created!', 'success');
          fetchAds();
        }
      }
    } catch (err) {
      if (drawerMode === 'edit' && targetId) {
        ...
        const newObj = { ...payload, _id: `bn-${Date.now()}`, impressions: 0, clicks: 0 };
  ...
  const handleDelete = (adObj) => {
    const targetId = adObj._id || adObj.id;
    showConfirm('Delete Banner Ad', 'Are you sure you want to delete this banner ad campaign?', async () => {
      try {
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold font-poppins text-slate-900 dark:text-white flex items-center gap-2">
            <ImageIcon className="w-6 h-6 text-brandPrimary" />
            <span>Banner Ad Management</span>
          </h2>
          <p className="text-xs text-slate-600 dark:text-white/50">
            Promotional banners displayed across homepage headers, sidebars, and contest screens.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 glassmorphism p-4 rounded-2xl border border-slate-200/80 dark:border-white/10 relative z-20">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search banner ads by title or sponsor..."
            value={searchTerm}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-6 gap-2 text-xs text-brandPrimary font-bold animate-pulse">
          ...
          <span>Loading Banner Ads...</span>
        </div>
      ) : filteredAds.length === 0 ? (
        <div className="glassmorphism p-12 rounded-3xl border border-slate-200/80 dark:border-white/10 text-center space-y-3">
          <ImageIcon className="w-10 h-10 text-slate-400 dark:text-white/30 mx-auto" />
          <h4 className="text-sm font-bold text-slate-800 dark:text-white">No Banner Ads Found</h4>
          <p className="text-xs text-slate-500 dark:text-white/40">Create a banner ad to start tracking impressions and click-through rates.</p>
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} BANNER AD`}>
        {drawerMode === 'view' ? (
          <div className="space-y-4 text-left text-xs">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">{formData.title}</h3>
            <p className="text-slate-500 dark:text-white/50 font-bold">Placement: {formData.placement}</p>
            {(formData.imageUrl || formData.mediaUrl) && (
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Banner Image</span>
                <img src={formData.imageUrl || formData.mediaUrl} alt="Ad Media" className="w-full h-40 object-cover rounded-xl border border-slate-200 dark:border-white/10" />
              </div>
            )}
            {formData.videoUrl && (
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Teaser Video</span>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-white/10 space-y-2">
              ...
              <div className="flex justify-between"><span>Placement:</span><span className="font-bold text-brandPrimary">{formData.placement}</span></div>
          ...
          <form onSubmit={handleSave} className="space-y-4 text-left text-xs">
            <div className="space-y-1.5">
              <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Banner Title *</label>
              <input
                type="text"
                required
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <CustomSelect
                  value={formData.placement || 'Home Screen Header'}
                  onChange={val => setFormData(prev => ({ ...prev, placement: val }))}
                  options={[
                    { label: 'Home Screen Header', value: 'Home Screen Header' },
                    { label: 'Contest Stage Banner', value: 'Contest Stage Banner' },
                    { label: 'Sidebar Slot', value: 'Sidebar Slot' },
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="https://example.com/banner-click"
                className="w-full bg-white dark:bg-[#0c1322] border border-slate-300 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white focus:outline-none"
              />
            </div>
            ...
            <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-white/5">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 flex items-center gap-1.5">
                  <ImageIcon className="w-3.5 h-3.5 text-brandPrimary" /> Banner Image Attachment
                </label>
                <FileUploadPicker
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 flex items-center gap-1.5">
                  <Video className="w-3.5 h-3.5 text-purple-500" /> Animated Video Banner (Optional)
                </label>
                <FileUploadPicker
                  folder="advertisements"
                  accept="video/*"
            ...
            <button type="submit" className="w-full py-3 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center justify-center gap-2 cursor-pointer shadow-lg mt-4">
              <Save className="w-4 h-4" /> Save Banner Ad
            </button>
          </form>
        )}
      </RightDrawer>
