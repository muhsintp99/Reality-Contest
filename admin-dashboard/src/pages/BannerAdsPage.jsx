...
import { Image, Plus, Search, Eye, Edit, Trash2, ToggleLeft, ToggleRight, RefreshCw, Save } from 'lucide-react';
...
export const BannerAdsPage = () => {
  ...
  const openDrawer = (mode, ad = null) => {
    ...
    setActiveAd(ad);
    if (mode === 'add') {
      setFormData({ title: '', type: 'banner', sponsorName: '', mediaUrl: '', redirectUrl: '', budget: 30000, placement: 'Home Screen Header', status: 'Active' });
    } else if (ad) {
      setFormData({ ...ad });
    }
    setDrawerOpen(true);
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold font-poppins text-slate-900 dark:text-white flex items-center gap-2">
            <Image className="w-6 h-6 text-brandPrimary" />
            <span>Banner Ads Management</span>
          </h2>
          <p className="text-xs text-slate-600 dark:text-white/50">
            Display image & animated banners on home screen, feed, and contest interstitial slots.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glassmorphism p-12 rounded-3xl border border-slate-200/80 dark:border-white/10 text-center space-y-3">
          <Image className="w-10 h-10 text-slate-400 dark:text-white/30 mx-auto" />
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} BANNER AD`}>
        {drawerMode === 'view' ? (
          <div className="space-y-4 text-left">
            ...
            <p className="text-xs text-slate-500">Placement: {formData.placement}</p>
          </div>
        ) : (
          <form onSubmit={handleSave} className="space-y-4 text-left">
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Image Asset URL</label>
              <input
                type="url"
