...
import { Trophy, Plus, Search, Eye, Edit, Trash2, ToggleLeft, ToggleRight, RefreshCw, Save, Image as ImageIcon, Video, ExternalLink } from 'lucide-react';
  ...
  const openDrawer = (mode, ad = null) => {
    ...
    if (mode === 'add') {
      setFormData({
        ...
        redirectUrl: '',
        budget: 100000,
        placement: 'Grand Contest Header',
        status: 'Active',
        description: ''
      });
    } else if (ad) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (drawerMode === 'edit' && targetId) {
        ...
        const newObj = { ...payload, _id: `sp-${Date.now()}`, impressions: 0, clicks: 0 };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => openDrawer('add')} className="px-4 py-2.5 bg-brandPrimary text-white rounded-xl text-xs font-bold hover:bg-brandPrimary/90 flex items-center gap-2 shadow-lg shadow-brandPrimary/15 cursor-pointer">
            <Plus className="w-4 h-4" /> Add Sponsored Contest
          </button>
        </div>
      </div>
      ...
      <RightDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title={`${drawerMode.toUpperCase()} SPONSORED CONTEST`}>
        {drawerMode === 'view' ? (
          <div className="space-y-4 text-left text-xs">
            ...
            <p className="text-slate-500 dark:text-white/50 font-bold">Sponsor: {formData.sponsorName || 'Brand Partner'}</p>
          ...
          <form onSubmit={handleSave} className="space-y-4 text-left text-xs">
            <div className="space-y-1.5">
              <label className="block text-[10px] text-slate-600 dark:text-white/40 uppercase font-bold">Contest Ad Title *</label>
              <input
                type="text"
                required
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <CustomSelect
                  value={formData.placement || 'Grand Contest Header'}
                  onChange={val => setFormData(prev => ({ ...prev, placement: val }))}
                  options={[
                    { label: 'Grand Contest Header', value: 'Grand Contest Header' },
                    { label: 'Contest Sidebar', value: 'Contest Sidebar' },
                    { label: 'Sponsor Popup', value: 'Sponsor Popup' }
                  ]}
                />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="https://sponsor.com/contest"
                className="w-full bg-white dark:bg-[#0c1322] border border-slate-300 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white focus:outline-none"
              />
            </div>
            ...
            <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-white/5">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 flex items-center gap-1.5">
                  <Video className="w-3.5 h-3.5 text-purple-500" /> Teaser Video Attachment
                </label>
                <FileUploadPicker
                  folder="advertisements"
                  accept="video/*"
