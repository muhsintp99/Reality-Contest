...
import { 
  ...
  Menu, ShieldAlert, Sparkles, User, Settings, HelpCircle
} from 'lucide-react';

export const Header = ({ activeView, onOpenMobileMenu, selectedRole, setSelectedRole }) => {
  ...
  const COMMAND_ITEMS = [
    { label: 'Go to Admin Console', action: () => navigate('/admin-dashboard/dashboard'), desc: 'Overview statistics and system summaries' },
    { label: 'Manage Users Directory', action: () => navigate('/admin-dashboard/users'), desc: 'Search and inspect registered users' },
    { label: 'Explore Contests Syndicate', action: () => navigate('/admin-dashboard/contests'), desc: 'Manage tournament entries and stages' },
    { label: 'Platform Security Settings', action: () => navigate('/admin-dashboard/settings'), desc: 'Configure system defaults' },
    ...
    <>
      <header className="sticky top-0 z-30 w-full px-6 py-4 flex items-center justify-between bg-[#0B1120]/65 dark:bg-[#0B1120]/65 light:bg-[#F8FAFC]/75 backdrop-blur-md border-b border-white/5 dark:border-white/5 light:border-black/5 transition-colors duration-300">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Simulator Role Selector Dropdown (Admin & Super Admin only) */}
          ...
          <div className="relative w-full max-w-xl bg-[#0F1424] light:bg-white border border-white/10 light:border-black/10 rounded-2xl shadow-2xl overflow-hidden z-10 p-2 animate-scale-in">
            ...
            <div className="max-h-60 overflow-y-auto space-y-1">
              {filteredCommands.length === 0 ? (
                <div className="p-4 text-center text-xs text-white/40">No matching links.</div>
              ) : (
